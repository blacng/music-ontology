...
| 10 | **ABox coverage report** (`make coverage`) | ✅ Done | `scripts/cq_coverage.py` — re-runs each CQ over TBox+ABox with fixtures excluded. Found CQ-8/11/15 green on fixtures alone, with **three different root causes**: a real data gap (CQ-8), an unrepresentative fixture asserting both halves of an `owl:inverseOf` pair (CQ-11), and sparse `:bornOn` (CQ-15). **17/17 now answerable** |
| 11 | **v2.4 — SHACL under RDFS inference** | ✅ Done | `check_shacl.py` ran `inference='none'`, so `sh:targetClass` never reached subclass instances. Now `inference='rdfs'` (no new dep — pyshacl ships owlrl; full `owlrl` rejected: it puns classes as individuals). That exposed the **`rdfs:domain` trap** — `rdfs:domain :MusicalArtist` on the career-onset properties is an *inference rule*, not a constraint, and was retyping producers/conductors as artists. Domains dropped; constraint moved to SHACL (`:CareerOnsetShape`, negative-tested). **17/17 CQs, SHACL 0/0, reasoner consistent** |
